...
class twitter_api(object):
    ...
    def __init__(self,string,consumer_key,consumer_secret,access_key,access_secret):
        ...
        #self.a=[p,n,l,j,h,f,d]
        #self.b=[n,l,j,h,f,d,y]
        #self.a=[h,f,d]
        #self.b=[f,d,y]
        #self.a=[f,d]
        #self.b=[d,y]
    ...
    def run(self):
        ...
        def createTestData(search_string,date,nextdate):
            try:
                ...
                print(date,nextdate)
                new_tweets=self.api.search(q=search_string,count=100,since=date,until=nextdate,result_type="recent")
                ...
                while (len(new_tweets) > 0):
                    ...
                    #for i in range(0,1):
                    new_tweets=self.api.search(q=search_string,count=100,max_id=oldest,since=date,until=nextdate,result_type="recent")
        ...
        for i in range(len(self.a)):
            testData=createTestData(search_string = self.string,date = self.a[i],nextdate = self.b[i])
            self.test.extend(testData)
        print ("Great! We fetched "+ str(len(self.test)) +" tweets for the term "+self.string + " !!")

        df = pd.DataFrame.from_dict(self.test)
        ...
        for index,row in df.iterrows():
            hashtags.append(str(self.test[index]["entities"][0]))
            user_mentions.append(str(self.test[index]["entities"][1]))
            ...
            Updated_source.append(str(df['Sources'][index]).partition('>')[-1].rpartition('<')[0])
        ...
        df['Issue'] = str(self.string)  #Defining the ISSUEs column as the search term itself
        ...
        columns = ['id','Date','shifted_date','tweet','RT Flag','language','user_id',
                      ...
                      'Influencers','Impressions','Topics']
        #df = df[columns]
